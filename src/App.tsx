...
const OptimizedEvents = () => (
  <Container maxWidth="lg" sx={{ py: 8, px: 4 }}>
    <Typography variant="h3" sx={{ mb: 4, fontWeight: 'bold' }}>Events</Typography>
    <Typography variant="h6" sx={{ mb: 6, color: 'text.secondary' }}>
      Upcoming events and workshops for veterans.
    </Typography>
    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 4 }}>
      {[
        'VA Claims Workshop',
        ...
        <Box key={event} sx={{ p: 4, backgroundColor: 'white', borderRadius: 2, boxShadow: 1 }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold', color: 'primary.main' }}>
            {event}
          </Typography>
          <Typography color="text.secondary">
            Join us for our upcoming {event.toLowerCase()}.
          </Typography>
        </Box>
      ))}
    </Box>
  ...
  return (
    <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      ...
      <Routes>
        ...
        <Route path="/events" element={<OptimizedEvents />} />
